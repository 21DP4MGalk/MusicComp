...
$stmnt = $connection->prepare("SELECT instruments.ID as iID, pieces.ID as pID, title, name, description, waveform FROM pieces LEFT JOIN instruments ON pieces.ID = instruments.pieceID WHERE userID = ? ORDER BY title;");
...
if($result->num_rows){
    for($i = 0; $i<$result->num_rows; $i++){
        ...
        $outputArray[] = [$row->title, $row->name, $row->description, $row->waveform, $row->iID, $row->pID];
    }
